# HEADER
Alexandre Balakirev  
Développeur Web PHP / Full-Stack  
LinkedIn: @alexandre-balakirev | https://linkedin.com/in/alexandre-balakirev  
Website: www.alex-balak.online | https://www.alex-balak.online

# PROFILE
Développeur **PHP / SQL** avec une expérience solide en conception d'API REST, architecture applicative et environnements orientés data. <br> À l'aise sans framework comme sur des architectures structurées (Composer, Laravel), autonome de la conception à la mise en production, rigoureux sur la qualité et la performance.

# CONTACT
- Téléphone : (+33) 06.58.37.06.05
- Email : alex.balak@outlook.fr
- Localisation : Lyon 8, France
- Permis B

# SKILLS
- [fa:brands:php] **PHP** : Architecture maison, Composer
- [fa:solid:database] **SQL** : SQL Server / MySQL / PostgreSQL
- [fa:solid:location-crosshairs] **API REST** : Conception, intégration
- [fa:brands:git-alt] Git : PR, revues de code
- [fa:brands:js] JavaScript / TypeScript : Frontend
- [fa:brands:react] React / Next.js : Frontend, Dev UX/UI
- [fa:brands:node-js] Node.js : Services backend, APIs
- [fa:brands:docker] Docker / Déploiement
- [fa:solid:wrench] Diagnostic, reproduction et correction
- [fa:brands:java] Java / Spring Boot : Backend, API REST

# CERTIFICATIONS
- [fa:brands:microsoft] Microsoft Python Developer
- [fa:brands:react] Meta React Developer
- [fa:solid:server] IBM Node / Express Developer
- [fa:solid:i] IBM Java Developer

# LANGUAGES
- Anglais — Courant (C1)
- Allemand — Notions
- Russe — Natif

# HOBBIES
- [fa:solid:dumbbell] Fitness / Musculation
- [fa:solid:person-biking] Ciclisme / Running



# EXPERIENCE

## Développeur Full-Stack PHP — Novocib
**Jul 2025 – Feb 2026**
- Développement backend PHP (Integartion Laravel, MySQL)
- Conception et maintenance d'API REST
- Optimisation de performances et requêtes SQL
- Participation aux revues de code

## Développeur SAGE X3 — Kardol
**Oct 2024 – Avr 2025**
- Développement ERP Sage X3 (L4G, SQL)
- Développement d'une application interne (Php, React, MySQL) 

## Développeur Full‑Stack PHP — Novocib
**Fév 2024 – Mai 2024**
- Développement Backend : PHP / MySQL
- Développement frontend : HTML, CSS, JS, Bootstrap
- Automatisation (Python, Powershell)

## Analyste Fonctionnel N2 — CGI
**Juil 2022 – Oct 2023**
- Diagnostic et résolution d'incidents
- Analyse de logs, reproduction d'anomalies
- Développement d'outils internes (Python, Bash)
- Travail en environnement critique (EDF)

## Technicien Avionique — Armée de Terre
**Mar 2010 – Juin 2021**
- Diagnostic de systèmes embarqués complexes
- Travail en anglais sur documentation technique

# EDUCATION

## Bac+2 : Développeur Web Full‑Stack
CMFP (AFPA), Fontenay-le-Comte  
**Août 2020 – Juin 2021**

## Licence Aéronautique
Armée de l'air, BA721  
**Juin 2011 – Juin 2012**